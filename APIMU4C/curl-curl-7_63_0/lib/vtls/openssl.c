               ...
               char *key_passwd)
{
  ...
  if(cert_file || (file_type == SSL_FILETYPE_ENGINE)) {
    ...
    printf(">> fuzzerr_apimu4c_curl_1\n");
      ...
      // bug curl2
    printf(">> fuzzerr_apimu4c_curl_2\n");
      BIO_read_filename(fp, cert_file);
                           ...
                           bool strict)
{
  ...
  if(rc) {
    ...
    if(SSL_SET_OPTION(issuercert)) {
      // bug curl10
        printf(">> fuzzerr_apimu4c_curl_10\n");
      fp = BIO_new(BIO_s_file());
      if(fp == NULL) {
        failf(data,
              "BIO_new return NULL, " OSSL_PACKAGE
